...
void merge_sort_td_merge(uint32_t *array, const uint32_t len,
                         uint32_t *worker_array) {
    uint32_t half     = len / 2;
    uint32_t i_left   = 0;
    ...
    // leftovers
    while (i_left < half) { worker_array[i_worker++] = array[i_left++]; }
...
void merge_sort_bu_merge(const uint32_t *array, const uint32_t start_index,
                         const uint32_t width, const uint32_t len,
                         uint32_t *worker_array) {
    ...
    // leftovers
    while (i_left < left_end) { worker_array[i_worker++] = array[i_left++]; }
    while (i_right < right_end) { worker_array[i_worker++] = array[i_right++]; }
}
