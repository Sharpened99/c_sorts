...
bool test_num_digits() {
    const std::array<pair<uint32_t, uint32_t>, 4> test_cases =
            {make_pair<uint32_t, uint32_t>(1, 1),
             ...
             make_pair<uint32_t, uint32_t>(312'860, 6)};

    return std::all_of(test_cases.begin(), test_cases.end(),
                       [](auto test_case) {
...
bool test_max_value() {
    const uint32_t values[9] = {123,  12,     2435,       3485, 2'397'479,
                                2384, 65'342, UINT32_MAX, 3848};

    return max_value(values, 9) == UINT32_MAX;
}

bool test_left_pad() {
    const std::array<std::string, 4> test_cases = {std::to_string(
                                                           (uint32_t) 13),
                                                   "hey",
